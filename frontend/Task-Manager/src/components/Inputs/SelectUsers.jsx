...
const SelectUsers = ({selectedUsers = [], setSelectedUsers}) => {
    ...
    useEffect(() => {
        if(selectedUsers?.length === 0) {
            ...
            // Initialize tempSelectedUsers with current selectedUsers when modal opens
            ...
            <div className='space-y-4 h-[60vh] overflow-y-auto'>
                {allUsers.map((user) => (
                    <div key={user._id} className='flex items-center gap-4 p-3 border-b border-gray-200'>
                        ...
                        <input 
                        type="checkbox"
                        checked={tempSelectedUsers?.includes(user._id) || false}
                        onChange={() => toggleUserSelection(user._id)} 
                        className='w-4 h-4 text-primary bg-gray-100 rounded-sm border border-gray-300 outline-none'
                        />
                    </div>
                ))}
            </div>
